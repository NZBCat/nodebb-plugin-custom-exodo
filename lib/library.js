(function (module) {
	...
	//hook que salta si se guarda el customizado en el usuario
	plugin.myMethod = function (data, callback) {
						 //console.log(data.uid);
						 //console.log(data.field);
						 //console.log("datos guadados");
	...
	Socket.saveUserCustomization = function (socket, data, callback) {
		...
		User.setUserField(socket.uid, "userCustomization", JSON.stringify(data.options), callback);
	};

	Socket.getUserCustomization = function (socket, data, callback) {
		//console.log(socket.uid);
		User.getUserField(socket.uid, "userCustomization", callback);
		//console.log(User.getUserData(2, callback));
		/* var usuarios = [];
		usuarios[0] = socket.uid;
		//console.log("hueheeh");
		//console.log(usuarios);
		User.getUsersData(usuarios, function(err, users) {
			console.log(JSON.stringify(users));
			callback(err, users ? users[0] : null);
		}); */
		//console.log(User.getUserField(socket.uid, 'username', callback));
		//User.getUserField(socket.uid, 'userCustomization', function(err, userCustomization) {
    //		console.log(JSON.stringify(userCustomization));
		//});
